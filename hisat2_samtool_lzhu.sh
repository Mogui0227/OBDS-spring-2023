...
#samtools --output=190314sb.out
#samtoolsH --error=190314sb.err

cd /project/obds/lzhu/1_linux/2_rnaseq/1_fastq/190314_K00198_0407_BH2CW7BBXY/
lines=$(find *_1.fastq.gz | sed "s/_1.fastq.gz//")

for i in ${lines}
do
        hisat2 --threads 4\
        -x /project/obds/lzhu/1_linux/2_rnaseq/2_genome/grch38/genome \
        -1 ${i}_1.fastq.gz \
        -2 ${i}_2.fastq.gz \
        --rna-strandness RF \
        --summary-file hisat2_run_${i}_stats.txt \
        -S ${i}.sam
